...
class TrackReconstuctor:
    ...
    def fit_track(self, i_event):
        self._file_data = r.DataFtame("tree", self._rootfile_path + "root")
        self._i_event = self._file_data.GetEntry(i_event)        
        self._hit_array = self._file_data.AsNumpy()
        for i_layer in range(-4, 4):
            for i in range(-2, 2):
                for j in range(-2, 2):
                    ...
                    if self._hit_array[i_layer][i + 2 ,j + 2] == 1.0:
